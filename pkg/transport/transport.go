...
type Config struct {
	...
	// ForceNewConn forces a brand new connection, bypassing the idle pool (v2.2.0+).
	// Used on retry attempts so a stale-connection failure is not retried on another
	// (potentially equally stale) idle connection. The per-host connection cap
	// (MaxConnsPerHost) is still honored via slot reservation.
	ForceNewConn bool
...
func (t *Transport) Connect(ctx context.Context, config Config, timer *timing.Timer) (net.Conn, *ConnectionMetadata, error) {
	...
	if config.ReuseConnection {
		if config.ForceNewConn {
			// Retry path: skip the idle pool entirely and dial a fresh connection,
			// but still respect the per-host connection cap via slot reservation.
			if !t.reserveSlot(poolKey) {
			...
			// Slot reserved, fall through to create a new connection.
		} else {
	...
	if maxConns > 0 && hp.numActive >= maxConns {
		...
		if t.poolConfig.WaitTimeout > 0 {
			...
			for hp.numActive >= maxConns {
				...
				// FIX (v2.1.2): sync.Cond.Wait() must be called in the same goroutine
				// that holds the lock. Wait() atomically unlocks the mutex, waits for
				// a signal, and relocks. Using a timer goroutine to Broadcast() on timeout.
				timer := time.AfterFunc(waitTime, func() {
					hp.cond.Broadcast()
				})

				// Wait for signal (properly handles unlock -> wait -> lock)
				hp.cond.Wait()

				// Check if we woke up due to timeout
				timedOut := !timer.Stop()

				// Check if idle connection became available
				if len(hp.idle) > 0 {
					n := len(hp.idle)
					pc := hp.idle[n-1]
					hp.idle = hp.idle[:n-1]
				...
				// If timed out and still no capacity, return timeout error
				if timedOut && hp.numActive >= maxConns {
					atomic.AddUint64(&t.statsWaitTimeouts, 1)
					return nil, nil, false
				}
				// Otherwise, loop continues to check conditions again
			}
		} else {
...
// reserveSlot reserves an active connection slot for a brand new connection without
// pulling from the idle pool. It honors MaxConnsPerHost (waiting up to WaitTimeout
// when configured). Returns true if a slot was reserved (caller must create and later
// release/close a connection carrying this poolKey), false if the pool is exhausted
// and the wait timed out. Used by the ForceNewConn retry path (v2.2.0+).
func (t *Transport) reserveSlot(key string) bool {
	hp := t.getOrCreateHostPool(key)
	...
	if maxConns > 0 && hp.numActive >= maxConns {
		...
		for hp.numActive >= maxConns {
			...
			timer := time.AfterFunc(waitTime, func() { hp.cond.Broadcast() })
			hp.cond.Wait()
			timedOut := !timer.Stop()
			if timedOut && hp.numActive >= maxConns {
				atomic.AddUint64(&t.statsWaitTimeouts, 1)
