...
type Options struct {
	...
	// ReadTimeout bounds how long a request waits for response frames (v2.2.0+).
	// Enforced per-request by the request goroutine (rolling: reset on each received
	// frame). 0 means no per-request timeout. Threaded from client.Options.ReadTimeout.
	...
	// EnableProtocolFallback is passed from client.Options (DEF-16, v2.1.4+).
	// Used internally to determine if fallback to HTTP/1.1 should occur on failure.
	EnableProtocolFallback bool

	// ProtocolExplicit indicates if Protocol was explicitly set by user.
...
type Stream struct {
	...
	// Multiplexing dispatch (v2.2.0+): the connection's single read loop routes
	// decoded frames for this stream into inbox. done is closed by the owning
	// request goroutine when it stops reading, so the read loop never blocks on a
	// finished/abandoned stream.
...
type Connection struct {
	...
	mu             sync.RWMutex // Protects connection state (fields above)

	// Multiplexing (v2.2.0+): a single read loop owns all reads from Framer and
	// dispatches frames to per-stream inboxes. writeMu serializes ALL Framer writes
	// (request frames, window updates, settings/ping ACKs). closedCh is closed when
	// the read loop terminates; connErr holds the terminal error. goAway* record a
	// received GOAWAY so streams beyond LastStreamID can be retried.
	writeMu            sync.Mutex
...
// Close gracefully closes the HTTP/2 connection (sends a GOAWAY).
func (c *Connection) Close() error {
	return c.shutdown(nil)
}

// fail tears the connection down due to a terminal error, recording cause as the
// connErr observed by waiting request goroutines. No GOAWAY is sent (the socket is
// assumed dead). Idempotent.
func (c *Connection) fail(cause error) {
	_ = c.shutdown(cause)
}

// shutdown is the single teardown path for the connection. It marks the connection
// closed, records the terminal error, closes closedCh to wake every waiting request
// goroutine, optionally emits a best-effort GOAWAY on graceful close, and closes the
// underlying socket (which unblocks the read loop). Idempotent.
func (c *Connection) shutdown(cause error) error {
	...
	// Best-effort GOAWAY only on graceful close; serialized via writeMu.
	if cause == nil && c.Framer != nil {
		c.writeMu.Lock()
...
// setReady marks the connection ready for use (SETTINGS handshake complete).
...
// isReady reports whether the connection is ready and not closed.
...
// markReused records that the connection was served from the pool.
func (c *Connection) markReused() {
	c.mu.Lock()
	c.Reused = true
	c.mu.Unlock()
}

// wasReused reports whether the connection was served from the pool.
func (c *Connection) wasReused() bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.Reused
}
